...
import { api } from '../api';
import { formatMatchScheduleLocal, parseMatchTimeUtc } from '../matchTime';
...
function actualSeriesForPair(idA, idB, alliances, playoffMatches) {
  ...
  const games = playoffMatches
    ...
    .slice()
    .sort((a, b) => {
      const ta = a.time ? parseMatchTimeUtc(a.time)?.getTime() ?? 0 : 0;
      const tb = b.time ? parseMatchTimeUtc(b.time)?.getTime() ?? 0 : 0;
      return ta - tb;
    });
